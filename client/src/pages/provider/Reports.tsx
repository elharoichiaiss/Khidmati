...
import { BarChart3, CalendarRange, Star, FileText, FileSpreadsheet, File, PieChart } from "lucide-react";
...
export default function ProviderReportsPage() {
    ...
    const reports = [
        ...
        },
        {
            ...
            description: t("bookingsReportDesc"),
            icon: CalendarRange,
    ...
    const download = (type: string, format: string) => {
        window.location.href = `/api/provider/report?type=${type}&format=${format}`;
    };
        ...
        <div className="bg-zinc-50 dark:bg-black min-h-screen py-10">
            <div className="container mx-auto px-4 max-w-5xl pb-24">
                <div className="flex items-center gap-3 mb-8">
                    <div className="p-3.5 bg-purple-50 dark:bg-purple-950/40 rounded-2xl text-purple-600 dark:text-purple-400">
                ...
                <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                    {reports.map((report) => (
                        <div key={report.type} className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800/80 shadow-[0_10px_30px_rgba(0,0,0,0.03)] p-6 hover:scale-[1.01] transition-transform" style={{ borderRadius: "28px" }}>
                            ...
                            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-5">{report.description}</p>
